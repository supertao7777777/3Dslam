...
void Preprocess::filterClosePoints(PointCloudXYZI &pl)
{
  if (blind <= 0.0 || pl.empty())
  {
    return;
  }
  ...
  for (const auto &pt : pl.points)
  {
    ...
    const double r2 = double(pt.x) * pt.x + double(pt.y) * pt.y + double(pt.z) * pt.z;
    if (r2 <= blind2)
    {
      continue;
    }
...
void Preprocess::process(const livox_ros_driver::CustomMsg::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out, pcl::PointCloud<pcl::PointXYZINormal>::Ptr &raw_cloud)
{  
  avia_handler(msg);
  customtopcNormal(msg,raw_cloud);
  *pcl_out = pl_surf; // 储存间隔采样点
}

void Preprocess::customtopcNormal(const livox_ros_driver::CustomMsg::ConstPtr &Msg, pcl::PointCloud<pcl::PointXYZINormal>::Ptr &raw_cloud){
  //只保存一帧
  raw_cloud->clear();
  raw_cloud->reserve(Msg->point_num);
  pcl::PointXYZINormal point;

  raw_cloud->header.frame_id=Msg->header.frame_id;
  raw_cloud->header.stamp=Msg->header.stamp.toNSec()/1000;
  raw_cloud->header.seq=Msg->header.seq;
  double timespan = Msg->points[Msg->point_num-1].offset_time*1e-9 - Msg->points[0].offset_time*1e-9;
  const double blind2 = blind * blind;
  ...
  for(uint i=0;i<Msg->point_num-1;i++)
  {
    ...
    point.normal_x=(Msg->points[i].offset_time*1e-9 - raw_cloud->header.stamp) / timespan; 

    if (blind > 0.0)
    {
      const double r2 = double(point.x) * point.x + double(point.y) * point.y + double(point.z) * point.z;
      if (r2 <= blind2)
      {
        continue;
      }
...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  // 统一再做一次近距离点剔除，避免不同分支/雷达类型下的条件差异导致漏滤。
  filterClosePoints(pl_surf);
}
...
void Preprocess::velodyne_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    if(feature_enabled)
    {
      ...
      for (int i = 0; i < plsize; i++)
      {
        ...
        // 防止异常 ring 值越界
        if (layer < 0 || layer >= N_SCANS || layer >= 128) continue;
        added_pt.x = pl_orig.points[i].x;
        added_pt.y = pl_orig.points[i].y;
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
