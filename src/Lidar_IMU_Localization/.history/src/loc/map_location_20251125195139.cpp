...
//   double ComputeError(const Eigen::Matrix4d &pose)
//   {
//     std::cout << "[FeatureLine::ComputeError] 开始计算误差" << std::endl;
    
//     // 1. 检查输入数据的有效性
//     std::cout << "[FeatureLine::ComputeError] 检查输入数据..." << std::endl;
//     if (!pose.allFinite()) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: 姿态矩阵包含 NaN 或 Inf!" << std::endl;
//       std::cout << "姿态矩阵:\n" << pose << std::endl;
//       error = 0;
//       valid = false;
//       return error;
...
//     if (!pointOri.allFinite()) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: pointOri 包含 NaN 或 Inf!" << std::endl;
//       std::cout << "pointOri: " << pointOri.transpose() << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//     if (!lineP1.allFinite()) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: lineP1 包含 NaN 或 Inf!" << std::endl;
//       std::cout << "lineP1: " << lineP1.transpose() << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//     if (!lineP2.allFinite()) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: lineP2 包含 NaN 或 Inf!" << std::endl;
//       std::cout << "lineP2: " << lineP2.transpose() << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
    
//     std::cout << "[FeatureLine::ComputeError] 输入数据检查通过" << std::endl;
    
//     // 2. 计算变换后的点
//     std::cout << "[FeatureLine::ComputeError] 计算变换后的点..." << std::endl;
//     Eigen::Vector3d P_to_Map = pose.topLeftCorner(3, 3) * pointOri + pose.topRightCorner(3, 1);
    
//     if (!P_to_Map.allFinite()) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: 变换后的点包含 NaN 或 Inf!" << std::endl;
//       std::cout << "P_to_Map: " << P_to_Map.transpose() << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
    
//     std::cout << "[FeatureLine::ComputeError] 变换后点: " << P_to_Map.transpose() << std::endl;
    
//     // 3. 计算线段长度
//     std::cout << "[FeatureLine::ComputeError] 计算线段长度..." << std::endl;
//     double dx = lineP1(0) - lineP2(0);
//     double dy = lineP1(1) - lineP2(1);
//     double dz = lineP1(2) - lineP2(2);
    
//     std::cout << "[FeatureLine::ComputeError] 线段分量差: dx=" << dx << ", dy=" << dy << ", dz=" << dz << std::endl;
    
//     double l12_sq = dx*dx + dy*dy + dz*dz;
//     std::cout << "[FeatureLine::ComputeError] 线段长度平方: " << l12_sq << std::endl;
    
//     // 检查平方值是否有效
//     if (std::isnan(l12_sq) || std::isinf(l12_sq) || l12_sq < 0) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: 线段长度平方无效: " << l12_sq << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
    
//     double l12 = std::sqrt(l12_sq);
//     std::cout << "[FeatureLine::ComputeError] 线段长度: " << l12 << std::endl;
    
//     // 防止除零
//     if (l12 < 1e-10) {
//       std::cout << "[FeatureLine::ComputeError] WARNING: 线段长度过小，跳过计算!" << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//     // 4. 计算叉乘面积
//     std::cout << "[FeatureLine::ComputeError] 计算叉乘面积..." << std::endl;
//     double term1 = (P_to_Map(0) - lineP1(0)) * (P_to_Map(1) - lineP2(1)) - 
//                    (P_to_Map(0) - lineP2(0)) * (P_to_Map(1) - lineP1(1));
//     double term2 = (P_to_Map(0) - lineP1(0)) * (P_to_Map(2) - lineP2(2)) - 
//                    (P_to_Map(0) - lineP2(0)) * (P_to_Map(2) - lineP1(2));
//     double term3 = (P_to_Map(1) - lineP1(1)) * (P_to_Map(2) - lineP2(2)) - 
//                    (P_to_Map(1) - lineP2(1)) * (P_to_Map(2) - lineP1(2));
    
//     std::cout << "[FeatureLine::ComputeError] 叉乘项: term1=" << term1 << ", term2=" << term2 << ", term3=" << term3 << std::endl;
    
//     double a012_sq = term1*term1 + term2*term2 + term3*term3;
//     std::cout << "[FeatureLine::ComputeError] 面积平方: " << a012_sq << std::endl;
    
//     // 检查平方值是否有效
//     if (std::isnan(a012_sq) || std::isinf(a012_sq) || a012_sq < 0) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: 面积平方无效: " << a012_sq << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
    
//     double a012 = std::sqrt(a012_sq);
//     std::cout << "[FeatureLine::ComputeError] 面积: " << a012 << std::endl;
    
//     // 5. 计算最终误差
//     std::cout << "[FeatureLine::ComputeError] 计算最终误差..." << std::endl;
//     error = a012 / l12;
    
//     // 检查最终误差是否有效
//     if (std::isnan(error) || std::isinf(error)) {
//       std::cout << "[FeatureLine::ComputeError] ERROR: 最终误差无效: " << error << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
    
//     std::cout << "[FeatureLine::ComputeError] 最终误差: " << error << std::endl;
//     valid = true;
    
//     std::cout << "[FeatureLine::ComputeError] 计算完成" << std::endl;
//     return error;
//   }
// };
...
//     // 添加严格的数值安全检查
//     if (!pose.allFinite()) {
//       std::cout << "[FeaturePlanVec] ERROR: Pose matrix contains NaN or Inf!" << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//     if (!pointOri.allFinite()) {
//       std::cout << "[FeaturePlanVec] ERROR: pointOri contains NaN or Inf!" << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
    
//     if (!pointProj.allFinite()) {
//       std::cout << "[FeaturePlanVec] ERROR: pointProj contains NaN or Inf!" << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//       std::cout << "[FeaturePlanVec] ERROR: sqrt_info matrix contains NaN or Inf!" << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//     // 检查变换后的点是否有效
//     if (!P_to_Map.allFinite()) {
//       std::cout << "[FeaturePlanVec] ERROR: Transformed point contains NaN or Inf!" << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//     // 计算误差向量
//     Eigen::Vector3d error_vec = P_to_Map - pointProj;
    
//     // 检查误差向量是否有效
//     if (!error_vec.allFinite()) {
//       std::cout << "[FeaturePlanVec] ERROR: Error vector contains NaN or Inf!" << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
...
//       std::cout << "[FeaturePlanVec] ERROR: Error norm is NaN or Inf: " << error_norm << std::endl;
//       error = 0;
//       valid = false;
//       return error;
//     }
