...
#define SAVE_TRAJ
  ...
  bool map_location::loadQRCodePosesFromFile(const std::string& file_path) {
  ...
  bool map_location::parseQRCodeLine(const std::string& line, QRCodeMapPose& qr_pose) {
  ...
  bool map_location::isCommentOrEmpty(const std::string& line) {
      // 检查是否为空行
      if (line.empty()) {
          return true;
      }
  ...
  bool map_location::getQRCodePose(int tag_id, QRCodeMapPose& result) const {
