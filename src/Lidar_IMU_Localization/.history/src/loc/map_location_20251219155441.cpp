...
private:
  ...
  // 二维码地图位姿存储（tag_id -> QRCodeMapPose）
  std::map<int, QRCodeMapPose> qrcode_map_poses_;
...
public:
  map_location()
  {
    ...
    //订阅二维码
    sub_QRcode = nh_.subscribe(QRcode_topic, 1 ,&map_location::QRcode_cbk,this);                       // 实例指针
...
#define SAVE_TRAJ
  ...
  bool loadQRCodePosesFromFile(const std::string& file_path) {
    ...
    while (std::getline(file, line)) {
        ...
        if (parseQRCodeLine(line, pose)) {
            ...
            valid_poses++;
            std::cout << "Loaded QR code pose for tag " << pose.tag_id << std::endl;
        } else {
    ...
    std::cout << "Total loaded " << valid_poses << " QR code poses from " 
              << file_path << std::endl;
  ...
  bool parseQRCodeLine(const std::string& line, QRCodeMapPose& qr_pose) {
    ...
    // 尝试读取8个double值 (tag_id x y z qx qy qz qw)
  ...
  void map_location::publishQRCodeMarkers() {
    ...
    for (const auto& pair : qrcode_map_poses_) {
        ...
        std::cout << "Published marker for QR code " << tag_id 
                  << " at position [" << qr_pose.position.x() << ", " 
                  << qr_pose.position.y() << ", " << qr_pose.position.z() << "]" << std::endl;
    }
  ...
  void map_location::publishQRCodeMarkersTimer(const ros::TimerEvent& event) {
      // 只有在有二维码数据时才发布
      if (!qrcode_map_poses_.empty()) {
          publishQRCodeMarkers();
      }
