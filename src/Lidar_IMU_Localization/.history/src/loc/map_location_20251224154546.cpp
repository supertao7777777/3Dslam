...
private:
  ...
  // 二维码回环因子
  struct QRCodeLoopFactor {
      EIGEN_MAKE_ALIGNED_OPERATOR_NEW
      
      QRCodeLoopFactor(const Eigen::Matrix4d& T_world_qr,
                        const Eigen::Matrix4d& T_robot_qr_obs,
                        const Eigen::Matrix<double, 6, 6>& sqrt_information)
          : T_world_qr_(T_world_qr), T_robot_qr_obs_(T_robot_qr_obs),
            sqrt_information_(sqrt_information) {}
      
      template<typename T>
      bool operator()(const T* const pose_ptr, T* residuals_ptr) const {
          // pose_ptr: 6维 [tx, ty, tz, rx, ry, rz] 旋转向量表示
          Eigen::Map<const Eigen::Matrix<T, 3, 1>> translation(pose_ptr);
          Eigen::Map<const Eigen::Matrix<T, 3, 1>> angle_axis(pose_ptr + 3);
          
          // 旋转向量转旋转矩阵
          T R[9];
          ceres::AngleAxisToRotationMatrix(angle_axis.data(), R);
          
          // 构建当前位姿
          Eigen::Matrix<T, 4, 4> T_world_robot = Eigen::Matrix<T, 4, 4>::Identity();
          for (int i = 0; i < 3; ++i) {
              for (int j = 0; j < 3; ++j) {
                  T_world_robot(i, j) = R[3*i + j];
              }
              T_world_robot(i, 3) = translation[i];
          }
          
          // 预测的二维码位姿
          Eigen::Matrix<T, 4, 4> T_robot_qr_obs_T = T_robot_qr_obs_.cast<T>();
          Eigen::Matrix<T, 4, 4> T_world_qr_pred = T_world_robot * T_robot_qr_obs_T;
          
          // 真实二维码位姿
          Eigen::Matrix<T, 4, 4> T_world_qr_true = T_world_qr_.cast<T>();
          
          // 计算误差
          Eigen::Matrix<T, 4, 4> T_error = T_world_qr_true.inverse() * T_world_qr_pred;
          
          // 平移误差
          Eigen::Matrix<T, 3, 1> trans_error = T_error.block<T, 3, 1>(0, 3);
          
          // 旋转误差（旋转向量）
          T R_error[9];
          for (int i = 0; i < 3; ++i) {
              for (int j = 0; j < 3; ++j) {
                  R_error[3*i + j] = T_error(i, j);
              }
          }
          
          T angle_axis_error[3];
          ceres::RotationMatrixToAngleAxis(R_error, angle_axis_error);
          
          // 构建6维残差
          Eigen::Map<Eigen::Matrix<T, 6, 1>> residuals(residuals_ptr);
          residuals.template head<3>() = trans_error;
          residuals.template tail<3>() = Eigen::Map<Eigen::Matrix<T, 3, 1>>(angle_axis_error);
          
          // 应用信息矩阵
          Eigen::Matrix<T, 6, 6> sqrt_info_T = sqrt_information_.cast<T>();
          residuals = sqrt_info_T * residuals;
          
          return true;
      }
      
      static ceres::CostFunction* Create(const Eigen::Matrix4d& T_world_qr,
                                          const Eigen::Matrix4d& T_robot_qr_obs,
                                          const Eigen::Matrix<double, 6, 6>& sqrt_information) {
          return new ceres::AutoDiffCostFunction<QRCodeLoopFactor, 6, 6>(
              new QRCodeLoopFactor(T_world_qr, T_robot_qr_obs, sqrt_information));
      }
      
  private:
      Eigen::Matrix4d T_world_qr_;
      Eigen::Matrix4d T_robot_qr_obs_;
      Eigen::Matrix<double, 6, 6> sqrt_information_;
  };
  
  // 二维码位置因子（只用xyz）
  struct QRCodePositionFactor {
      EIGEN_MAKE_ALIGNED_OPERATOR_NEW
      
      QRCodePositionFactor(const Eigen::Vector3d& world_pos,
                            const Eigen::Vector3d& robot_pos,
                            double weight = 50.0)
          : world_pos_(world_pos), robot_pos_(robot_pos), weight_(weight) {}
      
      template<typename T>
      bool operator()(const T* const pose_ptr, T* residuals_ptr) const {
          // pose_ptr: [tx, ty, tz, rx, ry, rz]
          Eigen::Map<const Eigen::Matrix<T, 3, 1>> translation(pose_ptr);
          Eigen::Map<const Eigen::Matrix<T, 3, 1>> angle_axis(pose_ptr + 3);
          ...
          // 将观测点转换到世界坐标系
          Eigen::Matrix<T, 3, 1> p_robot = robot_pos_.cast<T>();
          Eigen::Matrix<T, 3, 1> p_world_pred;
          
          for (int i = 0; i < 3; ++i) {
              p_world_pred[i] = R[3*i] * p_robot[0] + 
                                R[3*i + 1] * p_robot[1] + 
                                R[3*i + 2] * p_robot[2];
              p_world_pred[i] += translation[i];
          }
          
          // 计算误差
          Eigen::Map<Eigen::Matrix<T, 3, 1>> residuals(residuals_ptr);
          Eigen::Matrix<T, 3, 1> world_pos_T = world_pos_.cast<T>();
          residuals = (p_world_pred - world_pos_T) * T(weight_);
          
          return true;
      }
      
      static ceres::CostFunction* Create(const Eigen::Vector3d& world_pos,
                                          const Eigen::Vector3d& robot_pos,
                                          double weight = 50.0) {
          return new ceres::AutoDiffCostFunction<QRCodePositionFactor, 3, 6>(
              new QRCodePositionFactor(world_pos, robot_pos, weight));
      }
  ...
  // 二维码参数
  double qr_position_weight_;
  double qr_rotation_weight_;
  bool use_qr_loop_closure_;
  double qr_loop_closure_timeout_;
  
  // 二维码回环处理方法
  void addQRCodeLoopClosureFactor(ceres::Problem& problem,
                                  ceres::LossFunction* loss_function,
                                  double* pose_param,
                                  const QRcode& qr_obs);
  
  void addQRCodePositionFactor(ceres::Problem& problem,
                              ceres::LossFunction* loss_function,
                              double* pose_param,
                              const QRcode& qr_obs);
...
public:
  map_location()
  {
    ...
    //二维码参数
    nh_.param<double>("QRcode/position_weight", qr_position_weight_, 100.0);
    nh_.param<double>("QRcode/rotation_weight", qr_rotation_weight_, 50.0);
    nh_.param<bool>("QRcode/use_loop_closure", use_qr_loop_closure_, true);
    nh_.param<double>("QRcode/loop_closure_timeout", qr_loop_closure_timeout_, 5.0);
  ...
  void Estimate(std::list<LidarFrame> &frameList, const Eigen::Vector3d &gravity)
  {
    ...
    for (; iterOpt < max_iters; ++iterOpt)
    {
      ...
      for (int f = 1; f < windowSize; ++f)
      {
        ...
        std::advance(frame_curr, f);// frame_curr向前移动f帧
  ...
  void publishQRCodeMarkers() {
    ...
    std::cout << "Published " << marker_array.markers.size() 
              << " markers for " << qrcode_map_poses_.size() << " QR codes." << std::endl;
  }
  ...
  void map_location::addQRCodeLoopClosureFactor(ceres::Problem& problem,
                                             ceres::LossFunction* loss_function,
                                             double* pose_param,
                                             const QRcode& qr_obs) {
    ...
    // 查找地图中的二维码
    auto it = qrcode_map_poses_.find(qr_obs.tag_number);
    if (it == qrcode_map_poses_.end()) {
        std::cout << "QR code " << qr_obs.tag_number 
                  << " not found in map!" << std::endl;
        return;
    }
    
    const QRCodeMapPose& qr_map_pose = it->second;
    
    // 1. 构建相机坐标系下的二维码位姿
    Eigen::Matrix4d T_camera_qr = Eigen::Matrix4d::Identity();
    T_camera_qr(0, 3) = qr_obs.x;
    T_camera_qr(1, 3) = qr_obs.y;
    T_camera_qr(2, 3) = 0.0;  // 假设z=0
    
    // 创建绕z轴的旋转
    Eigen::AngleAxisd yaw_angle(qr_obs.yaw, Eigen::Vector3d::UnitZ());
    T_camera_qr.topLeftCorner(3, 3) = yaw_angle.toRotationMatrix();
    
    // 2. 转换到机器人坐标系
    Eigen::Matrix4d T_robot_camera = Eigen::Matrix4d::Identity();
    if (camera_robot_offset.size() >= 6) {
        // 平移
        T_robot_camera.topRightCorner(3, 1) = Eigen::Vector3d(
            camera_robot_offset[0], 
            camera_robot_offset[1], 
            camera_robot_offset[2]);
        
        // 旋转 (roll, pitch, yaw)
        Eigen::AngleAxisd roll(camera_robot_offset[3], Eigen::Vector3d::UnitX());
        Eigen::AngleAxisd pitch(camera_robot_offset[4], Eigen::Vector3d::UnitY());
        Eigen::AngleAxisd yaw(camera_robot_offset[5], Eigen::Vector3d::UnitZ());
        T_robot_camera.topLeftCorner(3, 3) = (yaw * pitch * roll).toRotationMatrix();
    }
    
    Eigen::Matrix4d T_robot_qr_obs = T_robot_camera * T_camera_qr;
    
    // 3. 地图中的二维码位姿
    Eigen::Matrix4d T_world_qr = qr_map_pose.toMatrix();
    
    // 4. 设置信息矩阵
    Eigen::Matrix<double, 6, 6> information = Eigen::Matrix<double, 6, 6>::Identity();
    information.block<3,3>(0,0) *= qr_position_weight_;    // 位置权重
    information.block<3,3>(3,3) *= qr_rotation_weight_;    // 旋转权重
    
    Eigen::Matrix<double, 6, 6> sqrt_information = information.llt().matrixL();
    
    // 5. 添加因子
    ceres::CostFunction* cost_function = QRCodeLoopFactor::Create(
        T_world_qr, T_robot_qr_obs, sqrt_information);
    
    problem.AddResidualBlock(cost_function, loss_function, pose_param);
    
    std::cout << ANSI_COLOR_GREEN << "[QR LOOP] Added SE(3) factor for QR " 
              << qr_obs.tag_number 
              << ", weights: pos=" << qr_position_weight_ 
              << ", rot=" << qr_rotation_weight_ 
              << ANSI_COLOR_RESET << std::endl;
  }

  void map_location::addQRCodePositionFactor(ceres::Problem& problem,
                                            ceres::LossFunction* loss_function,
                                            double* pose_param,
                                            const QRcode& qr_obs) {
      ...
      // 观测位置（机器人坐标系）
      Eigen::Vector3d robot_qr_obs(qr_obs.x, qr_obs.y, 0.0);
      
      // 应用相机外参
      if (camera_robot_offset.size() >= 6) {
          robot_qr_obs.x() += camera_robot_offset[0];
          robot_qr_obs.y() += camera_robot_offset[1];
          robot_qr_obs.z() += camera_robot_offset[2];
      }
      
      // 添加位置因子
      ceres::CostFunction* cost_function = QRCodePositionFactor::Create(
          qr_map_pose.position, robot_qr_obs, qr_position_weight_);
      
      problem.AddResidualBlock(cost_function, loss_function, pose_param);
      
      std::cout << ANSI_COLOR_GREEN << "[QR POS] Added position factor for QR " 
                << qr_obs.tag_number 
                << ", weight: " << qr_position_weight_ 
                << ANSI_COLOR_RESET << std::endl;
  }
