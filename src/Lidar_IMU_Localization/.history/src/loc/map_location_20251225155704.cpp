...
private:
  ...
  boost::shared_ptr<std::list<LidarFrame, Eigen::aligned_allocator<LidarFrame>>> lidarFrameList;
  std::string root_dir = ROOT_DIR;
...
public:
  map_location()
  {
    ...
    lidarFrameList.reset(new std::list<LidarFrame, Eigen::aligned_allocator<LidarFrame>>);
    ...
    while (true)
    {
      ...
      if (!vimuMsg.empty())
      {
        if (!LidarIMUInited)
        {
          ...
          lidar_list.reset(new std::list<LidarFrame, Eigen::aligned_allocator<LidarFrame>>);
