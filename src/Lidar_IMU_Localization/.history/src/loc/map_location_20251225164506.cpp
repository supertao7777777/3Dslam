...
private:
  ...
  using AlignedListLidarFrame = std::list<LidarFrame, Eigen::aligned_allocator<LidarFrame>>;
  // boost::shared_ptr<std::list<LidarFrame>> lidarFrameList;
  boost::shared_ptr<AlignedListLidarFrame> lidarFrameList;
  std::string root_dir = ROOT_DIR;
  ...
  struct SimpleQRCodeObservation {
    ...
    // 二维码约束权重参数（可在配置文件中调整）
    double qrcode_position_weight_ = 10.0;
    double qrcode_orientation_weight_ = 5.0;

    private:
    // ========== 6D二维码残差函数 ==========
    struct QRCodeSE3CostFunction {
        QRCodeSE3CostFunction(const Eigen::Vector3d& obs_position,
        ...
        bool operator()(const T* const pose, T* residuals) const {
            // pose: [tx, ty, tz, rx, ry, rz] (位置 + 旋转李代数)
            Eigen::Map<const Eigen::Matrix<T, 3, 1>> translation(pose);
            Eigen::Map<const Eigen::Matrix<T, 3, 1>> rotation_vec(pose + 3);
            
            // 使用李代数构造旋转矩阵
            Sophus::SO3<T> R_world_robot = Sophus::SO3<T>::exp(rotation_vec);
            
            // ========== 位置残差 ==========
            // 观测位置：从机器人坐标系变换到世界坐标系
            Eigen::Matrix<T, 3, 1> obs_world = 
                R_world_robot * obs_position_.cast<T>() + translation;
            
            // 位置误差（预测位置 - 地图位置）
            Eigen::Matrix<T, 3, 1> position_error = 
                obs_world - map_position_.cast<T>();
            
            // ========== 姿态残差 ==========
            // 观测姿态：从机器人坐标系变换到世界坐标系
            Sophus::SO3<T> R_robot_qr_obs(obs_orientation_.cast<T>());
            Sophus::SO3<T> R_world_qr_obs = R_world_robot * R_robot_qr_obs;
            
            // 地图中的二维码姿态
            Sophus::SO3<T> R_world_qr_map(map_orientation_.cast<T>());
            
            // 姿态误差：使用SO3的李代数（3维向量）
            Sophus::SO3<T> R_error = R_world_qr_map.inverse() * R_world_qr_obs;
            Eigen::Matrix<T, 3, 1> orientation_error = R_error.log();
            
            // ========== 加权残差 ==========
            // 前3个是位置残差，后3个是姿态残差
            for (int i = 0; i < 3; ++i) {
                residuals[i] = position_weight_ * position_error[i];
                residuals[i + 3] = orientation_weight_ * orientation_error[i];
            }
        ...
        static ceres::CostFunction* Create(const Eigen::Vector3d& obs_position,
                                          ...
                                          double position_weight = 1.0,
                                          double orientation_weight = 1.0) {
            return new ceres::AutoDiffCostFunction<QRCodeSE3CostFunction, 6, 6>(
                new QRCodeSE3CostFunction(obs_position, obs_orientation,
...
public:
  map_location()
  {
    ...
    nh_.param<double>("qrcode/position_weight", qrcode_position_weight_, 10.0);
    nh_.param<double>("qrcode/orientation_weight", qrcode_orientation_weight_, 5.0);
    ...
    lidarFrameList.reset(new AlignedListLidarFrame);
  ...
  void QRcode_cbk(const QRcode &msg_in)
  {
      ...
      current_qrcode_obs_.position = Eigen::Vector3d(msg_in.x, msg_in.y, 0);
      current_qrcode_obs_.orientation = Eigen::AngleAxisd(msg_in.yaw, Eigen::Vector3d::UnitZ());
      ...
      std::cout << "[QRCode] Detected tag " << msg_in.tag_number 
                << " at [" << msg_in.x << ", " << msg_in.y << ", 0]"
                << ", yaw: " << msg_in.yaw * 180.0 / M_PI << "°" << std::endl;
  }
  ...
  void vector2double(const AlignedListLidarFrame &tempFrameList)
  {
    int i = 0;
  ...
  void double2vector(AlignedListLidarFrame &tempFrameList)
  {
    int i = 0;
