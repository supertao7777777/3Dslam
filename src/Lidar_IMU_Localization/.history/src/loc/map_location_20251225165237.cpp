...
public:
  ...
  void Estimate(AlignedListLidarFrame &frameList, const Eigen::Vector3d &gravity)
  {
    TicToc etc;
    ...
    // ==========二维码约束==========
    // 存储二维码约束（每个约束包含代价函数和对应的帧索引）
    struct QRCodeConstraint {
        ceres::CostFunction* cost_function;
        int frame_index;
    };
    std::vector<QRCodeConstraint> qrcode_constraints;
    ...
    if (current_qrcode_obs_.valid) {
        ...
        for (auto frame_iter = frameList.begin(); 
             frame_iter != frameList.end(); 
             ++frame_iter, ++frame_idx) {
            
            double time_diff = std::abs(frame_iter->timeStamp - current_qrcode_obs_.timestamp);
        ...
        // 如果找到合适的时间匹配（100ms内）
        if (best_frame_index >= 0 && min_time_diff < 0.1) {
            // 在地图中查找对应的二维码位姿
            ...
            if (map_it != qrcode_map_poses_.end()) {
                ...
                // 创建6D二维码约束
                ceres::CostFunction* cost_function = 
                    QRCodeSE3CostFunction::Create(
                        current_qrcode_obs_.position,           // 观测位置
                        current_qrcode_obs_.orientation,        // 观测姿态
                        map_pose.position,      // 地图位置
                        map_pose.orientation,   // 地图姿态
                        qrcode_position_weight_,    // 位置权重
                        qrcode_orientation_weight_  // 姿态权重
                    );
                
                qrcode_constraints.push_back({cost_function, best_frame_index});
                
                std::cout << "[QRCode] Adding constraint for tag " 
                          << current_qrcode_obs_.tag_id 
                          << " to frame " << best_frame_index
                          << " (time diff: " << min_time_diff * 1000 << "ms)" 
                          << std::endl;
                ...
                current_qrcode_obs_.valid = false;  // 标记为无效，防止持续警告
            }
        } else if (best_frame_index >= 0) {
            std::cout << "[QRCode] Time diff too large: " << min_time_diff * 1000 
    ...
    for (; iterOpt < max_iters; ++iterOpt)
    {
      ...
      // ========== 添加二维码约束 ==========
      for (const auto& constraint : qrcode_constraints) {
          // 二维码约束通常很准确，不需要损失函数
          problem.AddResidualBlock(constraint.cost_function, nullptr, 
                                  para_PR[constraint.frame_index]);
      }
      
      // 二维码约束打印信息
      if (!qrcode_constraints.empty() && iterOpt == 0) {
          std::cout << "[QRCode] Added " << qrcode_constraints.size() 
                    << " QR code constraints to optimization" << std::endl;
      }
      ...
      if (iterOpt + 1 == max_iters) {
          // 最后一次迭代结束后，清理二维码约束
          for (auto& constraint : qrcode_constraints) {
              delete constraint.cost_function;  // 释放内存
          }
          qrcode_constraints.clear();
      }
    }
  ...
  void run()
  {
    ...
    while (true)
    {
      ...
      boost::shared_ptr<AlignedListLidarFrame> lidar_list;
      ...
      if (!vimuMsg.empty())
      {
        if (!LidarIMUInited)
        {
          ...
          lidar_list.reset(new AlignedListLidarFrame);
  ...
  bool ICPScanMatchGlobal(AlignedListLidarFrame &kframeList)
  {
