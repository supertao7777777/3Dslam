...
struct QRCodeSE3CostFunction {
    ...
    ExpSO3(const Eigen::MatrixBase<Derived>& w) {
        using T = typename Derived::Scalar;
        Eigen::Matrix<T, 3, 1> w_vec = w;  // 隐式转换到固定大小向量
        T theta = w_vec.norm();
        if (theta < T(1e-6)) {
            return Eigen::Matrix<T, 3, 3>::Identity();
        }
        Eigen::Matrix<T, 3, 1> axis = w_vec / theta;
        T c = cos(theta), s = sin(theta), t = T(1) - c;
    ...
    // 辅助函数：旋转矩阵到旋转向量（对数映射）
    template<typename Derived>
    static Eigen::Matrix<typename Derived::Scalar, 3, 1>
    LogSO3(const Eigen::MatrixBase<Derived>& R_) {
        using T = typename Derived::Scalar;
        Eigen::Matrix<T, 3, 3> R = R_;
        T tr = R.trace();
        T theta = acos((tr - T(1)) / T(2));
        if (theta < T(1e-6)) return Eigen::Matrix<T, 3, 1>::Zero();
        Eigen::Matrix<T, 3, 1> w;
        w(0) = (R(2,1) - R(1,2)) / (T(2) * sin(theta));
        w(1) = (R(0,2) - R(2,0)) / (T(2) * sin(theta));
        w(2) = (R(1,0) - R(0,1)) / (T(2) * sin(theta));
        return theta * w;
    }
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        // 使用指数映射构造旋转矩阵
        Eigen::Matrix<T, 3, 3> R_world_robot = ExpSO3(rotation_vec);
  ...
  void Estimate(std::list<LidarFrame> &frameList, const Eigen::Vector3d &gravity)
    {
      ...
      QRCodeConstraint qrcode_constraints;
    
    if (current_qrcode_obs_.valid) {
        // 总是使用最新帧（最后一帧）
        int best_frame_index = windowSize - 1;
        std::cout << "[QRCode] best_frame_index  " << best_frame_index << std::endl;
        if (best_frame_index >= 0) {
            ...
            if (map_it != qrcode_map_poses_.end()) {
                ...
                std::cout << "[QRCode] Found tag " << map_it->first
                << " in map at [" << map_it->second.position.transpose() << "]"
                << std::endl;
                ...
                qrcode_constraints.cost_function = cost_function;
                qrcode_constraints.frame_index = best_frame_index;
                
                std::cout << "[QRCode] Adding constraint for tag " 
                          << current_qrcode_obs_.tag_id 
                          << " to latest frame " << best_frame_index << std::endl;
                
                // 调试：打印当前帧的时间戳
                auto frame_iter = frameList.begin();
                std::advance(frame_iter, best_frame_index);
                std::cout << "[QRCode] Frame timestamp: " << frame_iter->timeStamp 
                          << std::endl;
            } else {
    ...
    for (; iterOpt < max_iters; ++iterOpt)
    {
      ...
      // ========== 添加二维码约束 ==========
      // 二维码约束通常很准确，不需要损失函数
      if (current_qrcode_obs_.valid == true){
        problem.AddResidualBlock(qrcode_constraints.cost_function, nullptr, 
                                para_PR[qrcode_constraints.frame_index]);
        // 二维码约束打印信息
        std::cout << "[QRCode] Added with " << qrcode_constraints.frame_index << std::endl;
        // 使用后立即置为无效，防止重复使用
        current_qrcode_obs_.valid = false;
      }
      ...
      if (iterOpt + 1 == max_iters) {
        ...
        if (qrcode_constraints.cost_function) {
        delete qrcode_constraints.cost_function;
        qrcode_constraints.cost_function = nullptr;   // 好习惯
    }
      }
    }
