...
struct QRCodeSE3CostFunction {
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        // 使用指数映射构造旋转矩阵
        Eigen::Matrix<T, 3, 3> R_world_robot;
        // Sophus::SO3<T>::exp(rotation_vec).matrix(R_world_robot);
        R_world_robot = Sophus::SO3<T>::exp(rotation_vec).matrix();
        ...
        Eigen::Matrix<T, 3, 3> R_world_qr_obs = R_world_robot * R_robot_qr_obs.matrix();
        
        // 地图中的二维码姿态
        Sophus::SO3<T> R_world_qr_map(map_orientation_.cast<T>());
        
        // 姿态误差：使用SO3的李代数（3维向量）
        Sophus::SO3<T> R_error = R_world_qr_map.inverse() * Sophus::SO3<T>(R_world_qr_obs);
