...
struct SimpleQRCodeObservation {
  ...
  struct QRCodeCostFunction {
      ...
      bool operator()(const T* const pose, T* residuals) const {
          ...
          Eigen::Matrix<T, 3, 1> position_error = obs_world - map_position_.cast<T>();
          ...
          // 计算旋转向量
          T trace = R_error.trace();
          T angle = acos((trace - T(1)) / T(2));
          Eigen::Matrix<T, 3, 1> orientation_error;
          
          if (angle < T(1e-6)) {
              orientation_error.setZero();
          } else {
              orientation_error(0) = (R_error(2,1) - R_error(1,2)) / (T(2) * sin(angle));
              orientation_error(1) = (R_error(0,2) - R_error(2,0)) / (T(2) * sin(angle));
              orientation_error(2) = (R_error(1,0) - R_error(0,1)) / (T(2) * sin(angle));
              orientation_error *= angle;
          }
          
          // ========== 加权残差 ==========
          residuals[0] = position_weight_ * position_error(0);
          residuals[1] = position_weight_ * position_error(1);
          residuals[2] = position_weight_ * position_error(2);
          residuals[3] = orientation_weight_ * orientation_error(0);
          residuals[4] = orientation_weight_ * orientation_error(1);
          residuals[5] = orientation_weight_ * orientation_error(2);
      ...
      static ceres::CostFunction* Create(const Eigen::Vector3d& obs_position,
                                        ...
                                        double position_weight = 1.0,
                                        double orientation_weight = 1.0) {
          return new ceres::AutoDiffCostFunction<QRCodeCostFunction, 6, 6>(
...
public:
  map_location()
  {
    ...
    nh_.param<double>("qrcode/position_weight", qrcode_position_weight_, 100000.0);
  ...
  void Estimate(std::list<LidarFrame> &frameList, const Eigen::Vector3d &gravity)
    {
      ...
      // 初始化空指针
      QRCodeConstraint qrcode_constraints{nullptr, -1};
    
    if (current_qrcode_obs_.valid)
    {
      // 总是使用最新帧（最后一帧）
      int new_frame_index = windowSize - 1;
      ...
      if (new_frame_index >= 0)
      {
        ...
        if (map_it != qrcode_map_poses_.end())
        {
          ...
          // ====== 调试：在加入约束前计算当前估计下的二维码误差（位置 + 姿态） ======
          auto frame_iter = frameList.begin();
          std::advance(frame_iter, new_frame_index);
          ...
          double trace = (R_error.trace() - 1.0) * 0.5;
          trace = std::max(-1.0, std::min(1.0, trace));  // 数值稳定
          double angle_rad = std::acos(trace);
          double angle_deg = angle_rad * 180.0 / M_PI;

          Eigen::Vector3d pos_error = obs_pos_world - map_pose.position;

          std::cout << "[QRCode][Debug] Pre-opt QR pos (world): " << obs_pos_world.transpose() << std::endl;
          std::cout << "[QRCode][Debug] Map   QR pos (world): " << map_pose.position.transpose() << std::endl;
          std::cout << "[QRCode][Debug] Position error (xyz / norm): "
                    << pos_error.transpose() << " / " << pos_error.norm() << " m" << std::endl;
          std::cout << "[QRCode][Debug] Orientation error: " << angle_deg << " deg" << std::endl;
          ...
          qrcode_constraints.frame_index = new_frame_index;

          std::cout << "[QRCode] Preparing constraint for tag "
                    << current_qrcode_obs_.tag_id
                    << " on frame " << new_frame_index << std::endl;
    ...
    for (; iterOpt < max_iters; ++iterOpt)
    {
      ...
      if (current_qrcode_obs_.valid == true){
          ...
          if (map_it != qrcode_map_poses_.end()) {
              ...
              std::cout << "[QRCode] Adding constraint for tag " 
                        << current_qrcode_obs_.tag_id 
                        << " to latest frame " << (windowSize - 1) << std::endl;
              std::cout << "[QRCode] Obs position: " << current_qrcode_obs_.position.transpose() << std::endl;
              std::cout << "[QRCode] Map position: " << map_pose.position.transpose() << std::endl;
              
              if (qrcode_constraints.cost_function == nullptr) {
                  std::cout << "[QRCode] ERROR: Failed to create cost function!" << std::endl;
              } else {
                  problem.AddResidualBlock(qrcode_constraints.cost_function, nullptr, 
                                          para_PR[windowSize - 1]);
      ...
      if (iterOpt + 1 == max_iters) {
        ...
        qrcode_constraints.cost_function = nullptr;   
        }
      }
    }
