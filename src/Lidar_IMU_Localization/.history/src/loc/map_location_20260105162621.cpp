...
public:
  map_location()
  {
    ...
    nh_.param<double>("QRcode/position_weight", qrcode_position_weight_, 20000000.0);
    nh_.param<double>("QRcode/orientation_weight", qrcode_orientation_weight_, 100.0);
    std::cout << "[QRCode] qrcode_position_weight_ " << qrcode_position_weight_ << std::endl;
    // 初始化二维码观测
  ...
  void Estimate(std::list<LidarFrame> &frameList, const Eigen::Vector3d &gravity)
    ...
    for (; iterOpt < max_iters; ++iterOpt)
    {
      ...
      if (current_qrcode_obs_.valid)
      {
        ...
        if (map_it != qrcode_map_poses_.end())
        {
          ...
          std::cout << "[QRCode][Debug] Position error (xyz / norm): "
                    << pos_error.transpose() << " / " << pos_error.norm()
                    << " m" << std::endl;
          std::cout << "[QRCode][Debug] Orientation error: "
      ...
      if (deltaR < 0.05 && deltaT < 0.05 || (iterOpt + 1) == max_iters)
      {
        ...
        if (current_qrcode_obs_.tag_id >= 0)
        {
