...
void IMUIntegrator::PreIntegration(double lastTime, const Eigen::Vector3d &bg, const Eigen::Vector3d &ba)
{
  ...
  double current_time = lastTime;
    ...
    // ========== 保持与状态传播一致的重力方向 ==========
  // 假设状态传播使用负重力（Z轴向下）
  // Eigen::Vector3d gravity_world(0, 0, 9.805);  // 负重力（与状态传播一致）
  // Eigen::Vector3d gravity_imu = dq.inverse() * gravity_world;
  // ========== 修改结束 ==========
  ...
  for (auto &imu : vimuMsg)
  {
    ...
    // // 更新重力方向
    // gravity_imu = dq.inverse() * gravity_world;
