...
private:
  ...
  // If true: publish map->odom using external odom->base_link TF (move_base friendly).
  // If false: publish map->base_link directly (legacy behavior).
  bool publish_map_to_odom_tf_ = false;

  // TF timestamp handling:
  // Some simulators/drivers may produce sensor stamps that jump ahead of ROS time (/clock),
  // which would trigger TF "extrapolation into the future" errors. When that happens,
  // clamp TF publish/lookup time to ros::Time::now().
  double tf_future_tolerance_sec_ = 0.2;
  ...
  // Debug: diagnose "path drift" while robot is stationary.
  bool debug_drift_ = false;
  double debug_drift_throttle_sec_ = 1.0;
  double debug_drift_trans_warn_m_ = 0.05;
  double debug_drift_yaw_warn_deg_ = 1.0;
  bool have_dbg_last_pose_ = false;
  Eigen::Vector3d dbg_last_p_ = Eigen::Vector3d::Zero();
  Eigen::Quaterniond dbg_last_q_ = Eigen::Quaterniond::Identity();
  double dbg_last_t_ = -1.0;
  double corner_leaf_;
  double surf_leaf_;
...
public:
  map_location()
  {
    ...
    nh_.param<std::string>("location/map_frame", map_frame_, std::string("world"));
    nh_.param<std::string>("location/odom_frame", odom_frame_, std::string("odom"));
    nh_.param<std::string>("location/base_frame", base_frame_, std::string("base_link"));
    nh_.param<std::string>("gps/topic", gps_topic_, "/gps/fix");
    nh_.param<int>("location/IMU_Mode", IMU_Mode, 0);
    nh_.param<bool>("location/use_imu_z", use_imu_z_, true);
    nh_.param<bool>("location/use_lio", use_lio, false);
    nh_.param<bool>("location/debug_drift", debug_drift_, false);
    nh_.param<double>("location/debug_drift_throttle_sec", debug_drift_throttle_sec_, 1.0);
    nh_.param<double>("location/debug_drift_trans_warn_m", debug_drift_trans_warn_m_, 0.05);
    nh_.param<double>("location/debug_drift_yaw_warn_deg", debug_drift_yaw_warn_deg_, 1.0);
    nh_.param<bool>("location/publish_downsampled_global_map", publish_downsampled_global_map_, false);
    nh_.param<bool>("location/publish_tf", publish_tf_, true);
    nh_.param<bool>("location/publish_map_to_odom_tf", publish_map_to_odom_tf_, false);
    nh_.param<bool>("location/publish_path", publish_path_, true);
    nh_.param<bool>("location/publish_odom_imu", publish_odom_imu_, true);
    nh_.param<double>("location/tf_future_tolerance_sec", tf_future_tolerance_sec_, 0.2);
    ...
    std::cout << "[location] IMU_Mode=" << IMU_Mode
              << ", use_imu_z=" << (use_imu_z_ ? "true" : "false") << std::endl;
    std::cout << "[location] debug_drift=" << (debug_drift_ ? "true" : "false")
              << ", throttle_sec=" << debug_drift_throttle_sec_
              << ", warn_trans_m=" << debug_drift_trans_warn_m_
              << ", warn_yaw_deg=" << debug_drift_yaw_warn_deg_
              << std::endl;
    std::cout << "[location] publish_tf=" << (publish_tf_ ? "true" : "false")
              << ", publish_map_to_odom_tf=" << (publish_map_to_odom_tf_ ? "true" : "false")
              << ", publish_path=" << (publish_path_ ? "true" : "false")
              << ", publish_odom_imu=" << (publish_odom_imu_ ? "true" : "false")
              << std::endl;
    std::cout << "[tf] tf_future_tolerance_sec=" << tf_future_tolerance_sec_ << std::endl;
    std::cout << "[frames] map_frame=" << map_frame_
    ...
    while(pub_corner_map.getNumSubscribers() == 0){
      ...
      msg_corner_target.header.frame_id = map_frame_;
  ...
  void imu_callback(const sensor_msgs::ImuConstPtr &imu_msg)
  {
    ...
    sensor_msgs::ImuPtr imu_filtered(new sensor_msgs::Imu(*imu_msg));
    if (!use_imu_z_)
    {
    ...
    _imuMsgQueue.push_back(imu_filtered);

    double t = imu_filtered->header.stamp.toSec();
    if (t <= 0.0)
      return;
...

    {
      ...
      Eigen::Vector3d w(imu_filtered->angular_velocity.x,
                        imu_filtered->angular_velocity.y,
                        imu_filtered->angular_velocity.z);
    ...
    publishImuOdom(imu_filtered->header.stamp, p, q, v);
  }
  ...
  bool fetchClosestGps(double t, GpsMeas &out, double *best_dt_out = nullptr)
  {
    if (!use_gps_)
      return false;
  ...
  static double yawDegFromQuat(const Eigen::Quaterniond &q)
  {
    Eigen::Vector3d ypr = q.toRotationMatrix().eulerAngles(2, 1, 0);
    return ypr[0] * 180.0 / M_PI;
  }

  void debugMaybePrintPoseDelta(const char *src, double t, const Eigen::Vector3d &p, const Eigen::Quaterniond &q)
  {
    if (!debug_drift_)
      return;

    if (!have_dbg_last_pose_ || dbg_last_t_ <= 0.0)
    {
      have_dbg_last_pose_ = true;
      dbg_last_p_ = p;
      dbg_last_q_ = q;
      dbg_last_t_ = t;
      return;
    ...
    const double dt = t - dbg_last_t_;
    const Eigen::Vector3d dp = p - dbg_last_p_;
    const Eigen::Quaterniond dq = dbg_last_q_.conjugate() * q;
    const double dyaw_deg = std::atan2(2.0 * (dq.w() * dq.z() + dq.x() * dq.y()),
    ...
    oss << "[drift] src=" << src
        ...
        << " |dp|=" << dp.norm()
        << " dyaw_deg=" << dyaw_deg
        << " yaw_deg=" << yawDegFromQuat(q)
        << " IMU_Mode=" << IMU_Mode
        << " qr_accepted=" << (last_qrcode_accepted_ ? "true" : "false")
        << " use_gps=" << (use_gps_ ? "true" : "false")
        << " use_lio=" << (use_lio ? "true" : "false");

    ROS_INFO_STREAM_THROTTLE(debug_drift_throttle_sec_, oss.str());
    if (dp.norm() > debug_drift_trans_warn_m_ || std::fabs(dyaw_deg) > debug_drift_yaw_warn_deg_)
      ROS_WARN_STREAM_THROTTLE(debug_drift_throttle_sec_, oss.str());
  ...
  ros::Time sanitizeTfStamp(double t_sec, const char *context)
  {
    ros::Time now = ros::Time::now();
    if (t_sec <= 0.0)
      return now;

    ros::Time stamp = ros::Time().fromSec(t_sec);

    // If /clock is not active yet, now may be zero; in that case just use the message stamp.
    if (!now.isZero() && stamp > (now + ros::Duration(tf_future_tolerance_sec_)))
    {
      ROS_WARN_THROTTLE(1.0,
                        "[loc] %s: input stamp %.6f is ahead of ros::Time::now() %.6f by > %.3fs. "
                        "Clamping to now to avoid TF extrapolation.",
                        context, stamp.toSec(), now.toSec(), tf_future_tolerance_sec_);
      return now;
    }
    return stamp;
  }
  ...
  void pubOdometry(Eigen::Matrix4d pose, double &time)
  {
    ...
    ros::Time ros_time = sanitizeTfStamp(time, "pubOdometry(mat4)");
    if (time <= 0)
      ROS_WARN_THROTTLE(1.0, "[loc] pubOdometry(mat4): Using current time instead of invalid timestamp: %f", time);
    ...
    if (publish_map_to_odom_tf_)
    {
      ...
      try
      {
        tf_listener_.lookupTransform(odom_frame_, base_frame_, ros_time, odom_to_base);
        tf::Transform map_to_odom = transform * odom_to_base.inverse();
        if (publish_tf_)
          broadcaster_.sendTransform(tf::StampedTransform(map_to_odom, ros_time, map_frame_, odom_frame_));
      }
      catch (tf::ExtrapolationException &ex)
      {
        // Common in sim: sensor stamp can be ahead of /clock, or TF buffer may not yet have that time.
        // Retry with the latest available TF to keep the tree connected for move_base.
        try
        {
          tf_listener_.lookupTransform(odom_frame_, base_frame_, ros::Time(0), odom_to_base);
          tf::Transform map_to_odom = transform * odom_to_base.inverse();
          // Publish with a "current" stamp (already clamped) so consumers like move_base
          // can query TF at now. The transform content uses the latest available odom->base.
          ros::Time stamp = ros_time;
          ROS_WARN_THROTTLE(1.0,
                            "[loc] publish_map_to_odom_tf=true but TF lookup extrapolation (%s->%s) at t=%.6f: %s. "
                            "Retrying with latest TF at t=%.6f.",
                            odom_frame_.c_str(), base_frame_.c_str(), ros_time.toSec(), ex.what(), odom_to_base.stamp_.toSec());
          if (publish_tf_)
            broadcaster_.sendTransform(tf::StampedTransform(map_to_odom, stamp, map_frame_, odom_frame_));
        }
        catch (tf::TransformException &ex2)
        {
          ROS_WARN_THROTTLE(1.0,
                            "[loc] publish_map_to_odom_tf=true but TF lookup failed (%s->%s) at t=%.6f: %s. "
                            "Falling back to publishing %s->%s.",
                            odom_frame_.c_str(), base_frame_.c_str(), ros_time.toSec(), ex2.what(),
                            map_frame_.c_str(), base_frame_.c_str());
          if (publish_tf_)
            broadcaster_.sendTransform(tf::StampedTransform(transform, ros_time, map_frame_, base_frame_));
        }
      }
      catch (tf::TransformException &ex)
      {
        ROS_WARN_THROTTLE(1.0,
                          "[loc] publish_map_to_odom_tf=true but TF lookup failed (%s->%s) at t=%.6f: %s. "
                          "Falling back to publishing %s->%s.",
                          odom_frame_.c_str(), base_frame_.c_str(), ros_time.toSec(), ex.what(),
                          map_frame_.c_str(), base_frame_.c_str());
        if (publish_tf_)
          broadcaster_.sendTransform(tf::StampedTransform(transform, ros_time, map_frame_, base_frame_));
      }
    ...
    laserPose.header.frame_id = map_frame_;
    ...
    laserOdoPath.header.frame_id = map_frame_;
    if (publish_path_)
      pubLaserOdometryPath_.publish(laserOdoPath);

    updateExtrapAnchor(Eigen::Vector3d(pose(0, 3), pose(1, 3), pose(2, 3)), Q, ros_time.toSec());
    debugMaybePrintPoseDelta("loc/pubOdometry(mat4)", ros_time.toSec(),
                             Eigen::Vector3d(pose(0, 3), pose(1, 3), pose(2, 3)), Q);
  }
  ...
  {
    ros::Time ros_time = sanitizeTfStamp(frame.timeStamp, "pubOdometry(frame)");
    ...
    if (publish_map_to_odom_tf_)
    {
      ...
      {
        tf_listener_.lookupTransform(odom_frame_, base_frame_, ros_time, odom_to_base);
        tf::Transform map_to_base = static_cast<tf::Transform>(transform_);
    ...
    debugMaybePrintPoseDelta("loc/pubOdometry(frame)", frame.timeStamp, frame.P, frame.Q);
  }
  ...
  void Estimate(std::list<LidarFrame> &frameList, const Eigen::Vector3d &gravity)
    {
      ...
      if (debug_drift_)
      {
        const auto &kf = frameList.back();
        const size_t in_sz = (kf.laserCloud ? kf.laserCloud->size() : 0);
        const size_t corner_sz = (kf.corner ? kf.corner->size() : 0);
        const size_t surf_sz = (kf.surf ? kf.surf->size() : 0);
        const size_t map_corner_sz = (map.globalCornerMapCloud_ ? map.globalCornerMapCloud_->size() : 0);
        const size_t map_surf_sz = (map.globalSurfMapCloud_ ? map.globalSurfMapCloud_->size() : 0);
        const size_t local_corner_sz = (laserCloudCornerFromLocal ? laserCloudCornerFromLocal->size() : 0);
        const size_t local_surf_sz = (laserCloudSurfFromLocal ? laserCloudSurfFromLocal->size() : 0);
        ROS_INFO_STREAM_THROTTLE(debug_drift_throttle_sec_,
                                 "[drift][cloud] in=" << in_sz
                                                     << " corner=" << corner_sz
                                                     << " surf=" << surf_sz
                                                     << " map(corner=" << map_corner_sz
                                                     << ", surf=" << map_surf_sz << ")"
                                                     << " local(corner=" << local_corner_sz
                                                     << ", surf=" << local_surf_sz << ")"
                                                     << " t=" << kf.timeStamp);
      }
    ...
    for (; iterOpt < max_iters; ++iterOpt)
    {
      ...
      int plane_blocks_added = 0;
      int imu_blocks_added = 0;
      int gps_blocks_added = 0;
      int qr_blocks_added = 0;
      int marg_blocks_added = 0;
      //初始化优化参数
      ...
      if (use_gps_)
      {
        for (int f = 0; f < windowSize; ++f)
        {
          ...
          GpsMeas gps_meas;
          double gps_dt = 0.0;
          if (fetchClosestGps(frame_curr->timeStamp, gps_meas, &gps_dt))
          {
      ...
      if (debug_drift_)
      {
        ...
        oss << "[drift][solve]"
            ...
            << " deltaR_deg=" << deltaR
            << " deltaT_m=" << deltaT
            << " blocks(line=" << line_blocks_added
            << ", plane=" << plane_blocks_added
            << ", imu=" << imu_blocks_added
            << ", gps=" << gps_blocks_added
            << ", qr=" << qr_blocks_added
            << ", marg=" << marg_blocks_added << ")"
            << " ceres(init_cost=" << summary.initial_cost
            << ", final_cost=" << summary.final_cost
            << ", iters=" << summary.iterations.size()
            << ", residuals=" << summary.num_residuals
            << ", params=" << summary.num_parameters << ")"
            << " valid(line=" << linevaild << ", surf=" << sufvaild << ")";
        ROS_INFO_STREAM_THROTTLE(debug_drift_throttle_sec_, oss.str());
      }
                          ...
                          const Eigen::Matrix4d &m4d)
  {
    ...
    const int laserCloudCornerStackNum = laserCloudCorner->points.size();

    int global_knn_ok = 0;
    int global_line_ok = 0;
    int global_edges = 0;
    ...
    const bool use_global = (laserCloudCornerGlobal && laserCloudCornerGlobal->points.size() > 100);
    const bool use_local = (laserCloudCornerLocal && laserCloudCornerLocal->points.size() > 20);
    ...
    if (debug_drift_)
    {
      ROS_INFO_STREAM_THROTTLE(debug_drift_throttle_sec_,
                               "[drift][assoc][line]"
                                   << " pts=" << laserCloudCornerStackNum
                                   << " thres_dist=" << thres_dist
                                   << " global(use=" << (use_global ? "true" : "false")
                                   << ", map_sz=" << (laserCloudCornerGlobal ? laserCloudCornerGlobal->size() : 0)
                                   << ", knn_ok=" << global_knn_ok
                                   << ", line_ok=" << global_line_ok
                                   << ", edges=" << global_edges << ")"
                                   << " local(use=" << (use_local ? "true" : "false")
                                   << ", map_sz=" << (laserCloudCornerLocal ? laserCloudCornerLocal->size() : 0)
                                   << ", knn_ok=" << local_knn_ok
                                   << ", line_ok=" << local_line_ok
                                   << ", edges=" << local_edges << ")");
    }
  }
                             ...
                             const Eigen::Matrix4d &m4d)
  {
    ...
    _matX0.setZero();
    const int laserCloudSurfStackNum = laserCloudSurf->points.size();

    int global_knn_ok = 0;
    int global_plane_ok = 0;
    int global_edges = 0;

    int local_knn_ok = 0;
    int local_plane_ok = 0;
    int local_edges = 0;

    int out_flag = 0; // global knn-ok count (historical behavior used by "map outside!")
    const bool use_global = (laserCloudSurfGlobal && laserCloudSurfGlobal->points.size() > 200);
    const bool use_local = (laserCloudSurfLocal && laserCloudSurfLocal->points.size() > 20);
    ...
    if (debug_drift_)
    {
      const double ratio = (laserCloudSurfStackNum > 0) ? (double(out_flag) / double(laserCloudSurfStackNum)) : 0.0;
      ROS_INFO_STREAM_THROTTLE(debug_drift_throttle_sec_,
                               "[drift][assoc][plane]"
                                   << " pts=" << laserCloudSurfStackNum
                                   << " global_knn_ratio=" << ratio
                                   << " thres_dist=" << thres_dist
                                   << " global(use=" << (use_global ? "true" : "false")
                                   << ", map_sz=" << (laserCloudSurfGlobal ? laserCloudSurfGlobal->size() : 0)
                                   << ", knn_ok=" << global_knn_ok
                                   << ", plane_ok=" << global_plane_ok
                                   << ", edges=" << global_edges << ")"
                                   << " local(use=" << (use_local ? "true" : "false")
                                   << ", map_sz=" << (laserCloudSurfLocal ? laserCloudSurfLocal->size() : 0)
                                   << ", knn_ok=" << local_knn_ok
                                   << ", plane_ok=" << local_plane_ok
                                   << ", edges=" << local_edges << ")");
    }
  ...
  bool ICPScanMatchGlobal(std::list<LidarFrame> &kframeList)
    ...
    *cloud_icp += *TransformPointCloud(surf, &initpose);
    ...
    msg_target.header.frame_id = map_frame_;
  ...
  bool extractSurroundKeyFrames(const PointType &p)
  {
    ...
    msg_surround_corner.header.frame_id = map_frame_;
    ...
    msg_surround_surf.header.frame_id = map_frame_;
  ...
  void publishQRCodeMarkers() {
    ...
    for (const auto& pair : qrcode_map_poses_) {
        ...
        arrow_marker.header.frame_id = map_frame_;
        ...
        cube_marker.header.frame_id = map_frame_;
